...
class Customer:
    ...
    def generate_customers(self, num_customers, restaurants):
        ...
        for _ in range(num_customers):
            ...
            customer_lead_source = random.choice(['Referral', 'Website', 'Promotional Ads'])
            customer_data = {
    ...
    def generate_marketing_data(self, restaurants):
        ...
        for restaurant in restaurants:
            ...
            for i in range(num_campaigns):
                ...
                platform = random.choice(['Email', 'Instagram', 'Facebook', 'Twitter'])
                post_id = ''
                total_like = ''
                total_comments = ''
                total_views = ''
                ...
                # If count is less than 10, keep budget low and marketing cost high
                if num_campaigns < 2:
...
class Expense:
    ...
    def generate_expense_data(self, restaurants, staff_data):
        expense_categories = ['Food', 'Utilities', 'Marketing', 'Maintenance', 'Salaries', 'Rent', 'Taxes', 'Insurance']
        ...
        for restaurant in restaurants:
            ...
            for category in expense_categories:
                ...
                if category == 'Salaries':
                    staff_salaries = sum([staff['Staff_Salary'] for staff in staff_data if staff['Restaurant_UID'] == restaurant_id])
                ...
                while current_date <= end_date:
                    expense_data = {
                        ...
                        'Total_Amount': round(expense_budget / 12, 2)  # Distribute equally for each month
                    }
                    self.expense_data.append(expense_data)
                    self.expense_id_counter += 1

                    # Move to the next month
                    current_date = (datetime.datetime.strptime(current_date, "%Y-%m-%d") + relativedelta(months=1)).strftime("%Y-%m-%d")
